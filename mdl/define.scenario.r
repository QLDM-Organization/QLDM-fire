...
define.scenario <- function(scn.name){
  ...
  fire.regime <-  read.table("inputfiles/FireRegime_Z13.txt", header=T)
  fire.sizes <- read.table("inputfiles/FireSizes_Z13.txt", header=T)
  sep.zone <- read.table("inputfiles/SEPzone_Z13C.txt", header=T)
  fuel.types.modif <- read.table("inputfiles/FuelTypeModif_Z13.txt", header=T)
  ...
  diff.prematurite <- 0   # for stands salvaged even if not mature (number of years before maturity)
  ...
  #spp.colonize.persist$persist <- c(0,0,0,0,0)
