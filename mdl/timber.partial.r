...
#  sum(even) 
 # even[land2$SppGrp %in% c("BOJ", "ERS", "OthDT")& is.na(land2$Exclus) & land2$rndm>0.95] <- 1
  ...
  #s.uea[s.uea$mgmt.unit=="8562",2]<- 0
  #as.data.frame(s.uea)
...
# management units, including those with no harvestable area (no mature uneven aged stands)
  
  units <- sort(unique(land2$mgmt.unit[!is.na(land2$mgmt.unit)]))
  units <- as.character(units[units != "AGNA"])
  
  ## A file that will contain sustainable yield per FMU
  recoltable.s <- cbind(as.data.frame(units), matrix(NA, nrow=length(units), ncol=hor.plan))
  names(recoltable.s)[1] <- "mgmt.unit"
  ...
  for(unit in units){
    ...
    if(dim(strate.fmu)[1]>0){  ## NÚ - The conditions length(strates)>0  does not make sense
                               ## I change to length(strate.fmu)>0
                               ## Mathieu - rechanged to dim(strates.fmu[1]>0)
      ...
      # in case there is no stratum available
      print(strate.fmu)
      print(unit)
      #print(recoltable.s[which(unit==recoltable.s$mgmt.unit),2:(hor.plan+1)])
      #print(colSums(recoltable)/(1:hor.plan)) # derniere iteration
      #recoltable.s[which(unit==recoltable.s$mgmt.unit),2:(hor.plan+1)] <-0  as.numeric(rep(0,hor.plan))
    }
  ...
  n.pc.cells <- data.frame(mgmt.unit=units, 
