...
landscape.dyn <- function(scn.name){
  ...
  for(irun in 1:nrun){
    ...
    ## feux: enregistrer les superficies a soustraire durant la p?riode suivante
    mem.feux <- rep(0,dim(zone.size)[1])
    
    #### persistence
      if(exists("persist")) {
            spp.colonize.persist[,4] <- ifelse(persist=="no.pers",0,1)
        }
    ...
    for(t in time.seq){
      ...
      if (t==time.seq[1] & is.clearcut)  {ref.harv.level = TS.CC.area}
      if (a.priori < 1 & is.clearcut) {
        if (replanif == 1) {
          TS.CC.area[,2] <- pmin(ref.harv.level[,2],TS.CC.area[,2]*a.priori)
        } else {
      ...
      if(timber.supply == "area.based" & is.clearcut & t %in% cc.schedule){      
        harv.out <- harvest.area(land, cc.step, diff.prematurite, hor.plan, TS.CC.area, TS.PC.area, salvage.rate.FMU,
                                 salvage.rate.event, km2.pixel, t, p.failure, age.seed, courbes, niveau.ref)
        cc.cells <- harv.out[[1]]
        pc.cells <- harv.out[[2]]
